...
- [ ] Manual testing with real agents (AC: 1-8)
  - [ ] Test with at least 2 registered agent phone numbers
  - [ ] Send "Hello Sophia" message from WhatsApp
  - [ ] Verify greeting response is received within 5 seconds
  - [ ] Verify response includes Sophia introduction and capabilities overview
  - [ ] Check `conversation_logs` table for correct entries (both inbound and outbound)
  - [ ] Test with 1 unregistered phone number
  - [ ] Verify unregistered agent receives polite rejection message
  - [ ] Test concurrent conversations (5 agents send messages simultaneously)
